...
class profileForm(FlaskForm):
    ...
    banner = TextAreaField(u"Write here what you're open to collaborate with",id="motto", validators=[DataRequired(),Length(min=20, max=200), CheckProfanity()])
    ...
    first_name = StringField('First Name', id="first-name", validators=[DataRequired(message="Please add your username"), Length(max=50), CheckProfanity()])
    
    last_name = StringField('Last Name', id="last-name", validators=[DataRequired(message="Please add your last name"), Length(max=50), CheckProfanity()])
    
    address = StringField(u'Address', id="address", validators=[Length(max=300), CheckProfanity()]) 
    
    country = HiddenField(id="country-input", validators=[DataRequired("Please add your country"), CheckProfanity()])
...
class ChangePassword(FlaskForm):
    password = PasswordField(id="old-password-input",validators=[DataRequired("Please input old password")])
    ...
    newPassword = PasswordField('Enter new Password', id="new-password-input", validators=[DataRequired("PLease input new password")])
